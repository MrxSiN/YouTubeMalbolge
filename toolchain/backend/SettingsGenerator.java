 ...
 */
final class SettingsGenerator implements Opcodes {
    static final String MEMBERS = P + "TargetMembers";
    private static final String ENTRY_UI = P + "SettingsEntry";
    ...
    private static final String NOTE =
            "Changes apply immediately to new screens and videos. Restart YouTube if one does not.";
    ...
    record Layouts(String row) {}
    record Section(String id, String title) {}
    record Item(String section, int configIndex, String title) {}
    record Settings(List<TargetMember> members, Map<String, Integer> roles, Page page, Layouts layouts,
    ...
    /** Hook handler: add the entry section after YouTube has (re)built its settings screen. */
    static void injectEntry(MethodVisitor m, GenerateModule.Hook hook) {
        m.visitVarInsn(ALOAD, 1);
        m.visitMethodInsn(INVOKEINTERFACE, GenerateModule.CHAIN, "proceed", "()Ljava/lang/Object;", true);
        m.visitVarInsn(ASTORE, 2);
        m.visitVarInsn(ALOAD, 1);
        m.visitMethodInsn(INVOKEINTERFACE, GenerateModule.CHAIN, "getThisObject", "()Ljava/lang/Object;", true);
        m.visitMethodInsn(INVOKESTATIC, ENTRY_UI, "inject", "(Ljava/lang/Object;)V", false);
        m.visitVarInsn(ALOAD, 2); m.visitInsn(ARETURN);
    }

    /** Cold-resolved table of exact target members used by the entry section. */
    ...
    private static void resolveField(MethodVisitor m, GenerateModule.Member b, String failure) {
        Label fail = new Label(), next = new Label();
        loadClass(m, b.owner(), 0); m.visitVarInsn(ASTORE, 1);
        m.visitVarInsn(ALOAD, 1);
        m.visitMethodInsn(INVOKEVIRTUAL, "java/lang/Class", "getSuperclass", "()Ljava/lang/Class;", false);
        m.visitLdcInsn(Type.getType("L" + b.superclass().replace('.', '/') + ";"));
        m.visitJumpInsn(IF_ACMPNE, fail);
        m.visitVarInsn(ALOAD, 1); m.visitLdcInsn(b.name());
        ...
        typeClass(m, Type.getType(b.descriptor()), 0);
        m.visitJumpInsn(IF_ACMPNE, fail);
        m.visitVarInsn(ALOAD, 2);
        ...
        m.visitLabel(fail); throwState(m, failure);
        m.visitLabel(next);
    ...
    private static void managerActivity(Path root, Settings s, List<GenerateModule.Config> configs,
        ...
        textView(m, "Waiting for YouTube hook report", 14f, false, 3, true);
        ...
        textView(m, "Hook report", 18f, true, 3, false);
        ...
        m.visitVarInsn(ALOAD, 3); m.visitVarInsn(ALOAD, 5);
        m.visitMethodInsn(INVOKEVIRTUAL, "android/widget/LinearLayout", "addView", "(Landroid/view/View;)V", false);
        textView(m, NOTE, 12f, false, 3, true);
        ...
        m.visitLdcInsn("YouTube Malbolge compatibility"); m.visitVarInsn(ALOAD, 0);
        ...
        for (int j = 0; j < s.items().size(); j++) {
            ...
            m.visitFieldInsn(GETFIELD, MANAGER, "hooksReady", "Z");
            m.visitMethodInsn(INVOKEVIRTUAL, "android/widget/Switch", "setEnabled", "(Z)V", false);
        }
        ...
        m.visitInsn(POP); m.visitInsn(RETURN);
        m.visitMaxs(0, 0); m.visitEnd();
        diagnosticMethods(w, s, hooks, active);
        save(root, MANAGER, w);
    }
    ...
    private static void diagnosticMethods(ClassWriter w, Settings s, List<GenerateModule.Hook> hooks,
                                          boolean active) {
        // locals: 0 activity, 1 snapshot, 2 status, 3 detail, 4/5 time, 6 builder, 7 package, 8 index, 9 error.
        MethodVisitor m = w.visitMethod(ACC_PUBLIC, "refreshStatus", "()V", null, null);
        m.visitCode();
        Label start = new Label(), end = new Label(), fail = new Label(), notReady = new Label();
        Label afterReady = new Label(), notStale = new Label(), checkModule = new Label(), noTime = new Label();
        m.visitTryCatchBlock(start, end, fail, "java/lang/Throwable");
        m.visitLabel(start);
        m.visitVarInsn(ALOAD, 0);
        m.visitMethodInsn(INVOKESTATIC, DiagnosticsGenerator.STATUS, "read",
        ...
        m.visitLdcInsn("com.google.android.youtube"); m.visitInsn(ICONST_0);
        ...
        m.visitFieldInsn(PUTFIELD, MANAGER, "hooksReady", "Z");
        m.visitLdcInsn("stale"); m.visitVarInsn(ASTORE, 2);
        m.visitLdcInsn("App or module changed since the last successful hook check. Restart YouTube.");
        m.visitVarInsn(ASTORE, 3);
        ...
        m.visitVarInsn(ASTORE, 6);
        append(m, "YouTube Malbolge compatibility\nStatus: ");
        m.visitVarInsn(ALOAD, 6); m.visitVarInsn(ALOAD, 2);
        ...
        append(m, "\nHooks active: ");
        m.visitVarInsn(ALOAD, 0); m.visitFieldInsn(GETFIELD, MANAGER, "hooksReady", "Z");
        m.visitJumpInsn(IFEQ, notReady);
        m.visitVarInsn(ALOAD, 6); integer(m, hooks.size());
        m.visitMethodInsn(INVOKEVIRTUAL, "java/lang/StringBuilder", "append",
                "(I)Ljava/lang/StringBuilder;", false); m.visitInsn(POP);
        m.visitJumpInsn(GOTO, afterReady);
        m.visitLabel(notReady); append(m, "0");
        m.visitLabel(afterReady); append(m, "/" + hooks.size() + "\nDetail: ");
        ...
        for (GenerateModule.Hook hook : hooks) {
            ...
            Label unavailable = new Label(), next = new Label();
            m.visitVarInsn(ALOAD, 0); m.visitFieldInsn(GETFIELD, MANAGER, "hooksReady", "Z");
            m.visitJumpInsn(IFEQ, unavailable); append(m, "installed\n");
            m.visitJumpInsn(GOTO, next);
        ...
        m.visitLdcInsn("Compatible: " + hooks.size() + "/" + hooks.size() + " hooks installed");
        ...
        if (active) {
            ...
            m.visitLdcInsn("Unavailable: open YouTube to check hooks");
            m.visitJumpInsn(GOTO, statusDone);
            m.visitLabel(hasStatus); m.visitLdcInsn("Unavailable: "); m.visitVarInsn(ALOAD, 2);
            m.visitMethodInsn(INVOKEVIRTUAL, "java/lang/String", "concat", "(Ljava/lang/String;)Ljava/lang/String;", false);
        } else m.visitLdcInsn("Unavailable: hooks disabled in this build");
        ...
        m.visitVarInsn(ALOAD, 0); m.visitMethodInsn(INVOKEVIRTUAL, MANAGER, "disableSwitches", "()V", false);
        m.visitVarInsn(ALOAD, 0); m.visitFieldInsn(GETFIELD, MANAGER, "statusView", "Landroid/widget/TextView;");
        m.visitLdcInsn("Unavailable: diagnostic read failed");
        m.visitMethodInsn(INVOKEVIRTUAL, "android/widget/TextView", "setText", "(Ljava/lang/CharSequence;)V", false);
